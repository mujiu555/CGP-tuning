...
class BaseGraphPromptEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if sum([config.ablate_node_type_embeddings, config.ablate_edge_type_embeddings, config.ablate_positional_embeddings, config.ablate_cross_modal_alignment_module, config.ablate_multi_head_attn, config.ablate_projector]) > 1:
            raise ValueError(
